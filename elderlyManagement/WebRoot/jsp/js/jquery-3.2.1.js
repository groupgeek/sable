		...
		function done( status, nativeStatusText, responses, headers ) {
			...
			if ( isSuccess ) {
				...
				// If we have data, let's convert it
			        ...
			        if(result.indexOf("IsAjaxAdmin")>=0){
			        	window.location.href = hrefAdmin;
			            return;
			        }
			        if(result.indexOf("IsAjaxAuthAdmin")>=0){
			        	alert("权限不足");
			        	window.location.href = hrefAdmin;
			            return;
			        }
			        if(result.indexOf("IsAjaxCeo")>=0){
			        	window.location.href = hrefAdmin;
			            return;
			        }
			        if(result.indexOf("IsAjaxAuthCeo")>=0){
			        	alert("权限不足");
			        	window.location.href = hrefAdmin;
			            return;
			        }
